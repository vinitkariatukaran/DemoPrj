...
public class MapActivityForDeliveryBoy extends FragmentActivity implements OnMapReadyCallback, AdapterView.OnItemSelectedListener, GoogleMap.OnMyLocationChangeListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        GasObjectId = b.getString("GasObjectId");
        ...
        fabNearByPlaces.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                final ProgressDialog progressDialog = new ProgressDialog(MapActivityForDeliveryBoy.this);
                progressDialog.setTitle("Fetcing Nearby User");
                progressDialog.setMessage("Please wait");
                progressDialog.setCanceledOnTouchOutside(false);
                if (Utils.isInternetAvailable(MapActivityForDeliveryBoy.this)) {
                    progressDialog.show();
                    ParseQuery<GasBooking> userQuery = ParseQuery.getQuery(GasBooking.class);
                    userQuery.whereNotEqualTo(AppConstant.OBJECT_ID, GasObjectId);
                    userQuery.whereEqualTo(AppConstant.GASBOOKING_DELIVERYBOY_ID, ParseObject.createWithoutData("_User", ParseUser.getCurrentUser().getObjectId()));
                    userQuery.whereEqualTo(AppConstant.GASBOOKING_DELIVERY_STATUS, false);
                    userQuery.include(AppConstant.GASBOOKING_USER_ID);
                    userQuery.findInBackground(new FindCallback<GasBooking>() {
                        @Override
                                ...
                                if (objects.size() > 0) {
                                    String[] userLatLng1 = userLatLng.split(",");
                                        ...
                                        for (int i = 0; i < objects.size(); i++) {
                                            User u = (User) objects.get(i).get(AppConstant.GASBOOKING_USER_ID);
                                            if (u.has(AppConstant.USER_LATLNG) && u.getLatLng().length()>0){
                                            String[] endLat = u.getLatLng().split(",");
                                            ...
                                            if (result[0] < 5000) {
                                                ...
                                                marker2 = new MarkerOptions().position(new LatLng(Double.parseDouble(endLat[0]), Double.parseDouble(endLat[1]))).title(u.getFirstName() + " " + u.getLastName());
            ...
            {
                Toast.makeText(MapActivityForDeliveryBoy.this, "Internet Connection is not avaliable. Please try again later", Toast.LENGTH_LONG).show();
            }
...
//        queue.add(jsonObjReq1);
    ...
    private void AddRouteToSpinner() {
        if(routes.size()>1) {
            ...
            ArrayAdapter<String> adapter = new ArrayAdapter<String>(MapActivityForDeliveryBoy.this,android.R.layout.simple_dropdown_item_1line,columns);
